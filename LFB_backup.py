...
item='''The data were loaded using the read_excel function from the pandas library. 
        ...
        These two datasets had three common identifiers, including the incidence number, CalYear(year of the call), HourOfCall (the hour of the call).
...
itemb = '''Table2. After merging the Incident dataframes, I realise that the full data consists
...
#a tree-based approach such as Random forest and an approach which combines linear and non-linear approacges 
...
st.image('imputation_comparison.png', caption='image from notebook')
...
item='''I would have expected different distributions of the data imputed from MICE, 
...
call_year = col1.number_input("CalYear", min_value = 0,
                                 value = int(x_lm['CalYear'].max()))

first_pump = col1.number_input("FirstPumpArriving_AttendanceTime", min_value = 0,
                                 value = int(x_lm['FirstPumpArriving_AttendanceTime'].max()))

turnout_sec = col2.number_input("TurnoutTimeSeconds", min_value = 0,
                                 value = int(x_lm['TurnoutTimeSeconds'].max()))

travel_time = col2.number_input("TravelTimeSeconds", min_value = 0,
                                 value = int(x_lm['TravelTimeSeconds'].max()))

const = 1

df = pd.DataFrame(columns=list([x_lm.columns]))
df.loc[0] = [const, call_year, first_pump, turnout_sec, travel_time]
turnout_sec
# Print the data
print(df)

# Use CalYear,FirstPumpArriving_AttendanceTime,TurnoutTimeSeconds,TravelTimeSeconds to predict FirstPumpArriving_AttendanceTime
prediction = lm_final.predict(df)

st.write("Your predicted attendance time is {0} seconds".format(round(prediction[0], 2)))
